...
locationsRouter.post('/', celebrate({ 
    ...
    const locationItems = items.map( async (item_id: Number) => {
            ...
            const selectedItem = await transaction('items').select('id').where('id',item_id).first();
...
locationsRouter.get('/', async (request, response) => {
    ...
    if (city && uf && items) {
        ...
        .where('location_items.item_id',parsedItems)
    ...
    return response.json({message: 'Locations not found.'});
});
